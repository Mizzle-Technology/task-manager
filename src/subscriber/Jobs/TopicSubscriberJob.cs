...
public class TopicSubscriberJob : IJob
{
    ...
    public async Task Execute(IJobExecutionContext context)
    {
        ...
        _logger.LogInformation(
            "Starting topic subscriber job at: {time} for topic {topic} subscription {subscription}",
            DateTimeOffset.Now,
            _config.TopicName,
        ...
        try
        {
            ...
            {
                _logger.LogInformation(
                    "No messages pulled from subscription {SubscriptionName}",
                    _config.SubscriptionName
                );
                return;
            }
            ...
            _logger.LogInformation(
                "Pulled {Count} messages from subscription {SubscriptionName}",
                messageList.Count,
                _config.SubscriptionName
            );
            ...
            var tasks = messageList.Select(message =>
                ProcessMessageSafeAsync(message, context.CancellationToken)
            );
        ...
        catch (Exception ex)
        {
            _logger.LogError(
                ex,
                "Error executing topic subscriber job for subscription {SubscriptionName}",
                _config.SubscriptionName
            );
            throw;
        }
    ...
    )
    {
        ...
        _logger.LogDebug(
            "Processing message {MessageId} from subscription {SubscriptionName}",
            messageId,
            _config.SubscriptionName
        );
    ...
    )
    {
        try
        {
            ...
            else
            {
                ...
                _logger.LogWarning(
                    "Message {MessageId} abandoned and returned to subscription: {Reason}",
                    message.MessageId,
                    reason
                );
            }
